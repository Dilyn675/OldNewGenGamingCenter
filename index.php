...
<head>
    ...
    <title>OldNewGen Gaming Center</title>
        ...
        <section class="games">
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo '<li>';
                    echo '<img src="' . $row['PictureLink'] . '" alt="' . $row['VideoGameName'] . '">';
                    echo '<h3>' . $row['VideoGameName'] . '</h3>';
                    echo '<p>Genre: ' . $row['Genre'] . '</p>';
                    echo '<p>Price: $' . $row['Price'] . '</p>';
                    echo '<a href="download.php?game_id='. $row['VideoGameID'].'">Download</a>';
                    echo '</li>';
                }
                echo '</ul>';
            } else {
